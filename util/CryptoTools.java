...
{8.12,1.49,2.71,4.32,12.02,2.3,2.03,5.92,7.31,0.1,0.69,3.98,2.61,6.95,7.68,1.82,0.11,6.02,6.28,9.1,2.88,1.11,2.09,0.17,2.11,0.07};
	 ...
	 **/
	public static double getIC(byte[] ar, int trials)
	{
	...
	public static double dotProduct(byte[] ar) {
		double dp = 0.0;
		int[] freqCount = CryptoTools.getFrequencies(ar);
			
		for (int i = 0; i < freqCount.length; i++)
			dp += ((0.0 + freqCount[i]) / ar.length) * (CryptoTools.ENGLISH[i] / 100);

		return dp;
	}

	public static double getFreqSquared() {
		double dotProduct = 0.0;
		
		for (int i = 0; i < ENGLISH.length; i++) {
			dotProduct += (ENGLISH[i] * ENGLISH[i]) / (100 * 100);
		}
		
		return dotProduct;
	}
	
	public static double getSumDistribution() {
		double sum = 0.0;
		
		for (double pct : ENGLISH) {
			sum += pct / 100;
		}
		
		return sum;
	}
	public static byte[] negateByteArray(byte[] input) {
		byte[] not = new byte[input.length];
	...
	public static byte[] xorByteArray(byte[] a1, byte[] a2) {
		byte[] xorResult = new byte[a1.length];
		for(int i =0; i<a1.length; i++) {
			xorResult[i] = (byte) (a1[i] ^ a2[i]);
		}
		return xorResult;
	}
